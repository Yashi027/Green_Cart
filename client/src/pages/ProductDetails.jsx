...
const ProductDetails = () => {
  ...
  const prod = product.find((item) => item._id === id);

  useEffect(() => {
    if (product.length > 0 && prod) {
      let productscopy = product.slice();
      productscopy = productscopy.filter((item) => prod.category === item.category);
      setRelatedProducts(productscopy.slice(0, 5));
    }
  }, [product, prod]);

  useEffect(() => {
    setThumbnail(prod?.image?.[0] || null);
  }, [prod]);
  ...
  return (
    <div className="max-w-6xl w-full px-6 mt-12">
      <p>
        ...
        <Link to={`/products/${prod?.category?.[0]?.toLowerCase()}`}> {prod?.category.join(",")}</Link> /
        <span className="text-primary"> {prod.name}</span>
      </p>
